...
export function WelcomeOnboarding({ onNavigate }: WelcomeOnboardingProps) {
    return (
        <div className="space-y-10 animate-in fade-in duration-700">
            ...
            {/* Quick Start Steps */}
            <div className="flex justify-center">
                <div className="flex flex-col md:flex-row gap-4 md:gap-6 max-w-4xl">
                    {STEPS.map((s, i) => (
                        <div key={i} className="flex items-start md:items-center gap-3 rounded-2xl bg-card/70 border border-border/70 px-4 py-3 shadow-sm flex-1">
                            ...
                            {i < STEPS.length - 1 && <ArrowRight className="w-4 h-4 text-muted-foreground/40 hidden md:block ml-auto" />}
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                {FEATURES.map((f) => (
                    <button
                        ...
                        className={`${f.bg} rounded-2xl p-5 text-left transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 border border-transparent hover:border-border/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring min-h-36`}
                    >
